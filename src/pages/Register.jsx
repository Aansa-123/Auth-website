...
export default function Register() {
  ...
  const { loading, error, registered, user, initializing } = useSelector((state) => state.auth)
  ...
  // Redirect to profile/admin if user is already logged in
  useEffect(() => {
        ...
        <div className="relative z-20 p-8 md:p-10">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {error && (
              <p className="text-red-500 text-sm font-medium">
                {typeof error === 'string' ? error : (error.error || error.message || 'Registration failed')}
              </p>
            )}
          ...
          <div className="text-center mt-6 pt-6 border-t border-gray-200">
            <p className="text-gray-600 text-sm">
              ...
              <Link to="/login" className="font-semibold text-purple-600 hover:text-pink-600 transition">
                Login
              </Link>
            </p>
          </div>
